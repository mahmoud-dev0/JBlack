...
import static GUI.DES_GUI.updateResultsTextArea;
...
public class DES {
	...
	public int[] permute(int[] inputBits, int[] keyBits, boolean isDecrypt) {
		...
		System.out.print("\nL0 = ");
		displayBits(L);
                updateResultsTextArea("\nL0 = "+displayBitsString(L));
		System.out.print("R0 = ");
		displayBits(R);
                updateResultsTextArea("R0 = "+displayBitsString(R));
		for(int n=0 ; n < 16 ; n++) {
			System.out.println("\n-------------");
			System.out.println("Round " + (n+1) + ":");
                        updateResultsTextArea("\n-------------");
                        updateResultsTextArea("Round " + (n+1) + ":");
			...
			if(isDecrypt) {
				...
				displayBits(subkey[15-n]);
                                updateResultsTextArea("Round key = "+displayBitsString(subkey[15-n]));
			} else {
				newR = fiestel(R, KS(n, keyBits));
				System.out.print("Round key = ");
				displayBits(subkey[n]);
                                updateResultsTextArea("Round key = "+displayBitsString(subkey[n]));
			}
			...
			System.out.print("L = ");
			displayBits(L);
                        updateResultsTextArea("L"+(n+1)+" = "+displayBitsString(L));
			System.out.print("R = ");
			displayBits(R);
                        updateResultsTextArea("R "+(n+1)+"= "+displayBitsString(R));
		}
		...
		if(isDecrypt) {
			System.out.print("Decrypted text: ");
                        updateResultsTextArea("Decrypted text: ");
		
		} else {
			System.out.print("Encrypted text: ");
                        updateResultsTextArea("Encrypted text: ");
		}
		System.out.println(hex.toUpperCase());
                updateResultsTextArea(hex.toUpperCase());
                finalOut += hex.toUpperCase();
		return finalOutput;
	}
	...
	public  static void displayBits(int[] bits) {
        ...
        public  String displayBitsString(int[] bits) {
		// Method to display int array bits as a hexadecimal string.
                String out="";
		for(int i=0 ; i < bits.length ; i+=4) {
			String output = new String();
			...
			out += Integer.toHexString(Integer.parseInt(output, 2))+" ";
		}
		return out;
	}
    
        public  int[] stringToBinary(String ss){
            ...
            for (int i = 0; i < 16; i++) {
                ...
                // string
                String s = Integer.toBinaryString(Integer.parseInt(ss.charAt(i) + "", 16));

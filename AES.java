...
/**
 ...
 AES alg = new AES(AES.KEY_SIZE_128); // up to 4096
...
public class AES {
    ...
    public static boolean isValidKeySize(int keySize) {
        if (keySize == AES.KEY_SIZE_128 ||
            keySize == AES.KEY_SIZE_192 ||
            keySize == AES.KEY_SIZE_256 ||
            keySize == AES.KEY_SIZE_512 ||
            keySize == AES.KEY_SIZE_1024 ||
            keySize == AES.KEY_SIZE_2048 ||
            keySize == AES.KEY_SIZE_4096) {
            return true;
        } else {
            return false;
        }
    ...
    public byte[] cipher(byte bytesMessage[], int wordsKeyExpansion[]) {
        CrypStego.updateResults("encrypting...");
        // create packet
        ...
        for (int i = 0; i < abk.size(); i++) {
         ...
         CrypStego.updateResults("encrypting complete");
    ...
    public void keyExpansion(byte key[], int w[]) {
        ...
        int c=1;
        for(int in=4;in<(Nb*(Nr+1));in+=4){
            String s="";
            for(int jn=in;jn<in+4;jn++){
                s+=Integer.toHexString(w[jn])+" ";
            }
            CrypStego.updateResults("k"+c+"= "+s);
    ...
    public byte[] invCipher(byte bytesMessage[], int wordsKeyExpansion[]) {
        CrypStego.updateResults("decrypting...");
        ...
        */
    	...
    	CrypStego.updateResults("decrypting complete");
        return bytesResult;
    }
